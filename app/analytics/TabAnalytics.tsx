...
export default function TabAnalytics() {
  ...
  const bmHistory = useMemo(() => {
    ...
    const baseS = spy.closes[0], baseQ = qqq?.closes[0] ?? 1, baseG = gld?.closes[0] ?? 1;
  ...
  const portfolioChartData = useMemo(() => {
    ...
    const baseSpy = filteredBm[0]?.sp500 ?? 0, baseQqq = filteredBm[0]?.nasdaq ?? 0, baseGld = filteredBm[0]?.gold ?? 0;
    ...
    return filtered.map((h) => {
      ...
      if (!bm && filteredBm.length) {
        ...
        while (lo < hi) { const mid = (lo + hi) >> 1; if (filteredBm[mid].date < h.date) lo = mid + 1; else hi = mid; }
        bm = filteredBm[lo];
      }
      return {
        date: h.date.slice(5),
  ...
  const liveFactorExposure = useMemo(() => {
    if (!app.portfolioTotal) return [
      { factor: "Market (Рынок)", value: 0.85 }, { factor: "Size (Размер)", value: 0.35 },
      { factor: "Value (Стоимость)", value: 0.30 }, { factor: "Growth (Рост)", value: 0.55 },
      { factor: "Quality (Качество)", value: 0.55 }, { factor: "Momentum (Моментум)", value: 0.45 },
    ...
    const FACTORS: Record<string, { market?: number; growth?: number; value?: number; quality?: number; momentum?: number; lowVol?: number; size?: number }> = {
      ...
      GLD:  { market: 0.1, lowVol: 0.3 }, SLV: { market: 0.15, lowVol: 0.2 }, IAU: { market: 0.1, lowVol: 0.3 },
      BTC:  { market: 0.3, growth: 0.7, momentum: 0.75 }, ETH: { market: 0.3, growth: 0.75, momentum: 0.7 },
    };
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 130px), 1fr))", gap: 10 }}>
        {[
          ...
          <div key={m.label} className="card" style={{ padding: 12 }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 700, color: (m as any).color }}>{m.value}</div>
            {(m as any).sub && <div style={{ fontSize: 11, color: "var(--text-secondary)" }}>{(m as any).sub}</div>}
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1.5fr 1fr 1fr", gap: 12 }}>
        <div className="card">
          <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", justifyContent: "space-between", alignItems: isMobile ? "flex-start" : "center", gap: isMobile ? 6 : 0, marginBottom: 8 }}>
            ...
            <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
              {periodOptions.map((p) => (
                <button key={p} className={`tab-btn ${period === p ? "active" : ""}`} onClick={() => setPeriod(p)} style={{ fontSize: 10, padding: "3px 7px" }}>{p}</button>
              ))}
            </div>
          </div>
        ...
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={160}>
            <PieChart>
              <Pie data={liveAssetDist} dataKey="value" cx="50%" cy="50%" innerRadius={45} outerRadius={70}>
