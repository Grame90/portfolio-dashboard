...
function buildPotentialDrawdown(annualVol: number, days: number) {
  ...
  for (let t = 0; t <= days; t++) {
    ...
    // VaR widens with sqrt(time) — negative = downside only
    result.push({
      date: label,
      band95:   parseFloat((-(1.645 * dailyVol * sqrtT)).toFixed(2)),
      band68:   parseFloat((-(1.000 * dailyVol * sqrtT)).toFixed(2)),
      expected: 0, // fair expected value ≈ 0 (markets unbiased)
    });
  }
  return result;
}

// ── Type volatility / beta estimates ──────────────────────────────────────
...
// Pairwise correlation by type (simplified)
...
export default function RisksPage() {
  ...
  const { metrics, riskScore, categories, stressScenarios, varRows,
    ...
    // Sharpe (simplified: annualised return / vol)
    ...
    const dailyChgPct = prevTotal > 0 ? (total - prevTotal) / prevTotal * 100 : 0;
    ...
    // Hedging instruments from portfolio
    ...
    // Cash
    if (cashVal > 0) hedgingRows.push({
      instrument: "Кэш (USD)", purpose: "Буфер ликвидности",
      share: cashShare.toFixed(1) + "%",
      recommendation: cashShare >= 5 ? "Держать" : "Увеличить",
    });
    // Metals
    for (const p of positions.filter(p => HEDGE_TICKERS.slice(0, 5).includes(p.ticker))) {
      const sh = (posValue(p) / total * 100).toFixed(1);
      hedgingRows.push({ instrument: `${p.name} (${p.ticker})`, purpose: "Защита от инфляции", share: sh + "%", recommendation: "Держать" });
    }
    // Bonds
    for (const p of positions.filter(p => HEDGE_TICKERS.slice(5).includes(p.ticker))) {
      const sh = (posValue(p) / total * 100).toFixed(1);
      hedgingRows.push({ instrument: `${p.name} (${p.ticker})`, purpose: "Защита от ставок", share: sh + "%", recommendation: "Держать" });
    }
    // Missing hedges suggestions
    const hasMetals = positions.some(p => HEDGE_TICKERS.slice(0, 5).includes(p.ticker));
    const hasBonds  = positions.some(p => p.type === "Облигация" || HEDGE_TICKERS.slice(5).includes(p.ticker));
    if (!hasMetals) hedgingRows.push({ instrument: "Золото (GLD)", purpose: "Защита от инфляции", share: "0%", recommendation: "Рассмотреть" });
    if (!hasBonds)  hedgingRows.push({ instrument: "Облигации (TLT)", purpose: "Защита от ставок", share: "0%", recommendation: "Рассмотреть" });
    if (cashShare < 5) hedgingRows.push({ instrument: "Inverse ETF (SH)", purpose: "Краткосрочный хедж", share: "0%", recommendation: "Рассмотреть" });

    // High risk positions: > 15% share or crypto, or drawdown > 20%
    const highRiskRows = positions
      .map(p => {
        const val = posValue(p);
    ...
    // Correlation matrix (top non-cash tickers, max 6)
    const topTickers = sorted.filter(p => p.type !== "Кэш").slice(0, 6).map(p => p);
  ...
  const riskAlerts = [
    ...
    {
      trigger: "Просадка портфеля (день)",
      current: `${app.dailyChangePct >= 0 ? "+" : ""}${app.dailyChangePct.toFixed(2)}%`,
      threshold: "< −3%", ok: app.dailyChangePct >= -3,
    },
    {
      trigger: "Концентрация (топ 5)",
      current: isEmpty ? "—" : `${top5pct.toFixed(1)}%`,
      threshold: "> 80%", ok: top5pct <= 80,
    },
    {
      trigger: "Доля кэша",
      current: isEmpty ? "—" : `${cashShare.toFixed(1)}%`,
      threshold: "< 5%", ok: cashShare >= 5,
    },
    ...
    <div style={{ minHeight: "100vh" }}>
      ...
      <div style={{ padding: "16px 24px", display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "0.8fr 1fr 1fr 0.8fr 1fr", gap: 12 }}>
          ...
          <div className="card">
            <div className="card-title">Потенциальная просадка (90 дней вперёд)</div>
            {isEmpty ? (
                ...
                <ResponsiveContainer width="100%" height={130}>
                  <AreaChart data={potentialDrawdown} margin={{ top: 8, right: 4, left: 0, bottom: 0 }}>
                    ...
                    <YAxis
                      ...
                      tickFormatter={(v) => `${v}%`}
                      domain={[Math.floor(potentialDrawdown[potentialDrawdown.length - 1]?.band95 ?? -20) - 2, 2]}
                      width={34}
                    />
                    <Tooltip
                      ...
                      formatter={(v: any, name: any) => [
                        `${Number(v).toFixed(2)}%`,
                        name === "band95" ? "VaR 95%" : name === "band68" ? "VaR 68%" : "Ожид.",
                      ]}
                    />
                    {/* 95% band — widest, red */}
                    <Area type="monotone" dataKey="band95" stroke="#ef4444" strokeWidth={1.5}
                          strokeDasharray="4 2" fill="url(#fan95)" dot={false} />
                    {/* 68% band — inner, amber */}
                    <Area type="monotone" dataKey="band68" stroke="#f59e0b" strokeWidth={1.5}
                          strokeDasharray="4 2" fill="url(#fan68)" dot={false} />
                    {/* Expected path — accent */}
                    <Area type="monotone" dataKey="expected" stroke="var(--accent)" strokeWidth={1.5}
          ...
          <div className="card">
            ...
            <table style={{ width: "100%", fontSize: 12, borderCollapse: "collapse" }}>
              ...
              <tbody>
                {(isEmpty ? [] : stressScenarios).map((s) => (
                  <tr key={s.scenario} style={{ borderBottom: "1px solid var(--border)" }}>
                    ...
                    <td style={{ padding: "7px 8px" }}>
                      <span className={`badge ${s.prob === "Низкая" ? "badge-green" : s.prob === "Высокая" ? "badge-red" : "badge-yellow"}`}>
                        {s.prob}
                      </span>
                    </td>
                  </tr>
                ...
                {isEmpty && (
                  <tr><td colSpan={5} style={{ padding: 16, textAlign: "center", color: "var(--text-muted)", fontSize: 12 }}>Нет данных портфеля</td></tr>
                )}
              </tbody>
            </table>
          </div>
                  ...
                  {hedgingRows.map((h, i) => (
                    <tr key={i} style={{ borderBottom: "1px solid var(--border)" }}>
                      ...
                      <td style={{ padding: "6px" }}>
                        <span style={{ color: h.recommendation === "Держать" ? "#22c55e" : h.recommendation === "Рассмотреть" ? "#f59e0b" : "#ef4444", fontWeight: 600 }}>
                          {h.recommendation}
                        </span>
                      </td>
                    </tr>
                ...
                <table style={{ borderCollapse: "collapse", fontSize: 10 }}>
                  ...
                  <tbody>
                    {correlMatrix.data.map((row, i) => (
                      <tr key={i}>
                        <td style={{ padding: "4px 5px", fontWeight: 700, color: "var(--text-secondary)", fontSize: 9 }}>
                          {correlMatrix.tickers[i]}
                        </td>
                        {row.map((val, j) => {
                          ...
                          return (
                            <td key={j} style={{
                              padding: "5px", textAlign: "center", fontSize: 10, fontWeight: 600,
                              background: bg,
                              color: abs > 0.5 ? "white" : "var(--text-primary)",
              ...
              <>
                <ResponsiveContainer width="100%" height={152}>
                  <AreaChart data={historicalDrawdown} margin={{ top: 8, right: 4, left: 0, bottom: 0 }}>
                    ...
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" vertical={false} />
                    <ReferenceLine y={0}   stroke="var(--text-muted)" strokeWidth={1} />
                    <ReferenceLine y={-5}  stroke="#f59e0b" strokeWidth={1} strokeDasharray="4 3" strokeOpacity={0.5}
                      label={{ value: "-5%", position: "insideTopRight", fill: "#f59e0b", fontSize: 8 }} />
                    ...
                    <YAxis
                      ...
                      tickLine={false} axisLine={false}
                      tickFormatter={(v) => `${v}%`}
                      domain={[Math.min(-15, Math.floor(maxHistDrawdown) - 2), 2]}
                      width={34}
                    />
                    <Tooltip
                      contentStyle={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, fontSize: 11 }}
                      labelStyle={{ color: "var(--text-secondary)", fontSize: 10 }}
                      formatter={(v: any) => [`${Number(v).toFixed(2)}%`, "Просадка от пика"]}
                      cursor={{ stroke: "#ef4444", strokeWidth: 1, strokeDasharray: "3 2", strokeOpacity: 0.4 }}
                    />
                    <Area type="monotoneX" dataKey="drawdown" stroke="#ef4444" strokeWidth={2}
                          fill="url(#ddGrad3)" dot={false}
                          activeDot={{ r: 4, fill: "#ef4444", stroke: "var(--bg-card)", strokeWidth: 2 }} />
                  </AreaChart>
                </ResponsiveContainer>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 0.8fr", gap: 12 }}>
          ...
          <div className="card">
            ...
            {(() => {
              ...
              const cashVal   = positions.filter(p => p.type === "Кэш").reduce((s, p) => s + posValue(p), 0);
              const etfVal    = positions.filter(p => p.type === "ETF").reduce((s, p) => s + posValue(p), 0);
              const stockVal  = positions.filter(p => p.type === "Акция").reduce((s, p) => s + posValue(p), 0);
              const cryptoVal = positions.filter(p => p.type === "Крипто").reduce((s, p) => s + posValue(p), 0);
